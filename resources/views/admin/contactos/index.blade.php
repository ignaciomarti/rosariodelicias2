    ...
    <div class="row panel-detalles justify-content-center">
        <table id="listado" class="table table-bordered table-hover w-100">
            ...
            <tbody id="myTable">
                ...
                <tr class="{{$contacto->status=="Finalizado"? "bg-success" : ""}} {{$contacto->status=="En seguimiento"? "bg-warning" : ""}}">
                    <td>{{$contacto->nombre}}</td>
                    <td>{{$contacto->email}}</td>
                    <td>{{$contacto->mensaje}}</td>
                    <td>{{$contacto->status}}</td>
                    <td> 
                        <form action="{{route('contactos.update', $contacto)}}" method="post" class="float-left">
                            @csrf
                            @method('PUT')
                        ...
                        <form action="{{route('contactos.destroy', $contacto)}}" method="post" class="float-left">
                            @csrf
                            @method('DELETE')
